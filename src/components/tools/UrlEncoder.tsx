...
export default function UrlEncoder() {
    ...
    const processData = (text: string, currentMode: "encode" | "decode") => {
        ...
        try {
            ...
            setOutput("⚠️ Error: Malformed URI or invalid encoded string.");
        }
    };
            ...
            <div className="grid gap-6 md:grid-cols-2">
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium tracking-tight text-muted-foreground">
                        {mode === "encode" ? "Plain Text URI" : "Encoded URI"}
                    </label>
                    <textarea
                        value={input}
                        onChange={(e) => processData(e.target.value, mode)}
                        placeholder={mode === "encode" ? "Enter URL or text to encode..." : "Enter URI encoded string to decode..."}
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                        <label className="text-sm font-medium tracking-tight text-muted-foreground">
                            {mode === "encode" ? "Encoded URI Output" : "Plain Text Output"}
                        </label>
                        <button
                            onClick={handleCopy}
