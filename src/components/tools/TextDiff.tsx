...
export default function TextDiff() {
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-border/50">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <button
                        onClick={handleSwap}
                        className="flex items-center justify-center gap-2 px-4 py-2 bg-slate-200 dark:bg-slate-800 text-foreground font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors shadow-sm flex-1 sm:flex-none"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-[400px]">
                ...
                <div className="flex flex-col h-full rounded-xl border border-border bg-card overflow-hidden shadow-sm">
                    <div className="px-4 py-3 border-b border-border/50 bg-slate-50 dark:bg-slate-900/50">
            ...
            {diffResult && (
                <div className="flex flex-col rounded-xl border border-border bg-card overflow-hidden shadow-sm mt-4">
                    <div className="px-4 py-3 border-b border-border/50 bg-slate-50 dark:bg-slate-900/50 flex items-center justify-between">
                        <span className="text-sm font-semibold">Comparison Result</span>
                        <div className="flex gap-4 text-xs font-medium">
                            <span className="flex items-center gap-1"><div className="w-3 h-3 rounded-sm bg-red-200 dark:bg-red-900/50 border border-red-300 dark:border-red-800"></div> Removed</span>
                            <span className="flex items-center gap-1"><div className="w-3 h-3 rounded-sm bg-green-200 dark:bg-green-900/50 border border-green-300 dark:border-green-800"></div> Added</span>
                        </div>
                    </div>
                    <div className="p-4 bg-transparent max-h-[500px] overflow-auto whitespace-pre-wrap font-mono text-sm leading-relaxed text-foreground">
                        {diffResult.map((part, index) => (
                            <span
                                key={index}
                                className={cn(
                                    part.added ? "bg-green-200 text-green-900 dark:bg-green-900/40 dark:text-green-300 py-0.5 px-1 rounded-sm mx-0.5" :
                                        part.removed ? "bg-red-200 text-red-900 dark:bg-red-900/40 dark:text-red-300 py-0.5 px-1 rounded-sm mx-0.5 line-through decoration-red-500/50" : "text-foreground"
                                )}
                            >
                                {part.value}
                            </span>
